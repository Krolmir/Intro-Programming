...
def get_line(town, strng)
  strng.split("\n").each do |line|
    temp = line.split(":")
    return line if temp[0] == town
  end
  return -1
end

def strip_line(line)
  line2 = line.gsub!(/[a-z]/i, '')
  line2 = line.gsub!(":", '')
  line2 = line.gsub!(" ", '')
  line2
end
...
def mean(town, strng)
  total = 0
  line = get_line(town, strng)
  return -1 if line == -1
  temp = strip_line(line)
  floats = temp.split(",").map { |str| str.to_f }
  total = floats.reduce(:+) / floats.size
  total
end
...
def variance(town, strng)
  ...
  # binding.pry
  avg = mean(town, strng)
  new_arr = floats.map { |num| (num - avg) ** 2 }
  total = new_arr.reduce(:+) / new_arr.size
  total  
end
...
p mean("Lon", data) # , 51.199999999999996) 
